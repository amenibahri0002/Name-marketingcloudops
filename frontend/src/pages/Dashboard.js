  ...
  const fetchData = async () => {
    try {
      ...
      setStats(statsRes.status === 'fulfilled' ? statsRes.value.data : {
        clients: 12, campagnes: 7, contacts: 1240, segments: 9,
        campagnesActives: 5, roas: 3.8, budgetConsomme: 12450
      });

      setEvolution(evoRes.status === 'fulfilled' ? evoRes.value.data : null);
      setCanaux(canauxRes.status === 'fulfilled' ? canauxRes.value.data : null);
      setRealtime(realtimeRes.status === 'fulfilled' ? realtimeRes.value.data : { usersOnline: 14 });
