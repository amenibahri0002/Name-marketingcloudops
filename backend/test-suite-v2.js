// test-suite-v2.js - Tests automatisés DigiPip
...
// ✅ APRÈS
assert([401, 403].includes(status), 'Status 401 ou 403, recu: ' + status);    
}
  });
...
async function testUsers() {
  ...
  await test('3.2 Liste users (Client - limite)', async () => {
    try {
      await api('GET', '/api/users', null, tokens.client, TEST_CONFIG.tenantId);
      assert(false, 'Devrait etre interdit');
    } catch (e) {
      assert(e.response?.status === 403, 'Status 403');
    }
  });
...
async function testCampagnes() {
  ...
  await test('4.3 Campagnes filtrees par tenant', async () => {
    ...
    try {
      ...
      assert(e.response?.status === 403, 'Status 403 pour fake tenant');
    }
  });
}
